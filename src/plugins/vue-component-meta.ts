...
export async function vueComponentMeta(
  ...
  return {
    name: 'storybook:vue-component-meta',
    setup(api) {
      api.transform(
        { test: (id) => filter(id) },
        async ({ code: src, resource: id }) => {
          try {
            ...
            let componentsMeta = exportNames.map((name) =>
              checker.getComponentMeta(id, name),
            );
            ...
            componentsMeta.forEach((meta, index) => {
              ...
              metaSources.push({
                ...
                displayName:
                  exportName === 'default'
                    ? getFilenameWithoutExtension(id)
                    : exportName,
                ...meta,
                exposed,
                sourceFiles: id,
            ...
            metaSources.forEach((meta) => {
              ...
              const name = isDefaultExport ? 'script' : meta.exportName;
  ...
  try {
    ...
    componentMeta.map((meta, index) => {
      ...
      meta.events = meta.events.map((event) => {
        ...
        if (description) {
          (event as typeof event & { description: string }).description =
            description;
        }
        return event;
      });
